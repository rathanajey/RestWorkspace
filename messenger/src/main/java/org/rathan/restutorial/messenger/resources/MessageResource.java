package org.rathan.restutorial.messenger.resources;
...
import org.rathan.restutorial.messenger.resources.beans.MessageFilterBean;
import org.rathan.restutorial.messenger.service.MessageService;
...
public class MessageResource {
	...
	public List<Message> getJsonMessages(@BeanParam MessageFilterBean filterBean) {
		...
		if(filterBean.getSize()>0 && filterBean.getStart()>=0) return messageService.getAllMessagesPaginated(filterBean.getStart(), filterBean.getSize());
	...
	public Message getMessage(@PathParam("messageId") long messageId, @Context UriInfo uriInfo){
		...
		String commentsUri=getUriForComments(uriInfo, message);
	...
	public Response addXmlMessage(Message message, @Context UriInfo uriInfo) throws URISyntaxException{
		System.out.println("XML POST method call");
		Message newMessage = messageService.addMessage(message);
		String newId=String.valueOf(newMessage.getId());
		URI uri=uriInfo.getAbsolutePathBuilder().path(newId).build();
		return Response.created(uri)
	...
	@DELETE
	@Path("/{messageId}")
	public void deleteMessge(@PathParam("messageId") long id){
